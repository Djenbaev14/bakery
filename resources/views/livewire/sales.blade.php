        ...
        <div class="form-group col-12">
          ...
          <select class="form-control client_id"  name="client_id" required onchange="myFunction(this,{{$breads}})">
            ...
            @foreach ($clients as $index=> $client)
                <option value="{{$client}}" class="pt-2">{{$client->name}}</option>
            @endforeach
          </select>
        </div>
...
{{-- <script>
  var client_id = document.getElementsByClassName('client_id');
  var quantity = document.getElementsByClassName('quan');
  var price = document.getElementsByClassName('price');
  var summa = document.getElementsByClassName('summa');

  function myFunction(client,breads,kindergarden){
