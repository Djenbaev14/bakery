 ...
 *
 ...
 $('a#add-education').cloneData({
            updateContainerId: 'main-container', // Main container Should be ID
            cloneUpdateContainer: 'clone-container', // Which you want to clone
            removeButtonClassUpdate: 'remove-education', // Remove button for remove cloned HTML
 ...
 */
        ...
        (function($) {
          ...
          $.fn.cloneDataUpdate = function(options, callback) {
      
              var settings = jQuery.extend({
                  updateContainerId: "clone-container",
                  cloneUpdateContainer: "clone-item",
                  excludeHTML: ".exclude",
                  emptySelector: ".empty",
                  copyClass: "clone-div",
                  removeButtonClassUpdate: "remove-item",
                  removeConfirm: false,
              ...
              //console.log('init called from library'+ $('#' + settings.updateContainerId).find('.'+settings.cloneUpdateContainer).length);
              settings.init.call({index: settings.counterIndex});
      
              var _addItem = function () {
      
                  settings.counterIndex = $('.' + settings.cloneUpdateContainer).length;
                  settings.beforeRender.call(this);
      
                  var item_exists = $('.' + settings.cloneUpdateContainer).length;
                  ...
                  $('#' + settings.updateContainerId).append(settings.template.first()[0].outerHTML);
              ...
              var _updateAttributes = function () {
                  ...
                  $('.' + settings.cloneUpdateContainer).each(function(index) {
                  ...
                  $('#' + settings.updateContainerId).addClass('clone-data');
                  $('#' + settings.updateContainerId + ' .' + settings.cloneUpdateContainer).each(function(parent_index, item){
                      $(this).attr('data-index', parent_index).addClass(settings.copyClass);
                  });
      
      
                  $('.' + settings.cloneUpdateContainer + '.' + settings.copyClass).each(function(parent_index, item) {
              ...
              var _updateAttrID = function($elem, index) {
                  ...
                  if (id !== newID) {
                      $elem.closest('.'+settings.cloneUpdateContainer).find('.field-' + id).each(function() {
                          $(this).removeClass('field-' + id).addClass('field-' + newID);
                      });
                      // update "for" attribute
                      $elem.closest('.'+settings.cloneUpdateContainer).find("label[for='" + id + "']").attr('for',newID);
                  }
                  ...
                  if (name !== undefined) {
                      ...
                      if (matches && matches.length === 4) {
                          ...
                          if (identifiers.length > 1) {
                              var widgetsOptions = [];
                              $elem.parents('.'+settings.updateContainerId).each(function(i){
                                  widgetsOptions[i] = eval($(this).find('#'+settings.updateContainerId));
                              });
      
                              widgetsOptions = widgetsOptions.reverse();
                              for (var i = identifiers.length - 1; i >= 1; i--) {
                                  identifiers[i] = $elem.closest('#'+settings.updateContainerId).index();
                              }
                          }
              ...
              var _parseTemplate = function() {
                  var template_clone = $('#' + settings.updateContainerId +' .' + settings.cloneUpdateContainer + ":first");
                  ...
                  if(!settings.defaultRender){
                      ...
                      $('.' + option.cloneUpdateContainer + ":first").remove();
                  }
              ...
              var _deleteItem = function($elem) {
                  ...
                  if (count > settings.minLimit) {
                      if(settings.removeConfirm){
                        $elem.parents('.' + settings.cloneUpdateContainer).slideUp(function(){
                            $(this).remove();
                            _updateAttributes();
              ...
              var _count = function() {
                  return $('.' + settings.cloneUpdateContainer).closest('#' + settings.updateContainerId).find('.'+settings.cloneUpdateContainer).length;
              };
      
      
              $(document).on('click', '.' + settings.removeButtonClassUpdate, function(){
